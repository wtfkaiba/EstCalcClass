...
def calcular_estatisticas(lim_inf_primeira, amplitude, qtd_classes, frequencias):
    ...
    df['Frequência Acumulada (Fac)'] = df['Frequência Absoluta (fi)'].astype(float).cumsum() # Assegurar float para cumsum
    ...
    if max_freq == 0 or (len(frequencias_abs_series.unique()) == 1 and N > 0) :
        ...
        else: estatisticas_resultados["Classificação Modal"] = "Polimodal" # Mais de 3 modas
    ...
    if df_resultados is None: 
        ...
        with tab_graficos:
            ...
            if soma_frequencias == 0: 
                ...
                y_ogiva = [0.0] + df_resultados['Frequência Acumulada (Fac)'].astype(int).tolist()
